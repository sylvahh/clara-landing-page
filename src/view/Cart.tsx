...
import { CART_EMPTY, arrowLeft, imageBaseUrl, numberWithCommas, trash } from '../utilities';
...
const Cart = () => {
  ...
  const { cartItems, cartUtils, cartTotal } = useStore()
...
useEffect(() => {
  ...
  const item =  cartItems.map((item: {product_id:string,  p_name: string, img: string, quantity:number, c_price:string, subTotal: number, }, idx) => {
    ...
    <td className='px-6 py-4 font-medium text-lg'>
      ...
      <td className='px-6 py-4'>
        ...
        <button disabled={item.quantity === 1} onClick={()=> handleQuantity(item.product_id,'DECREMENT')} className=' font-semibold text-lg'>-</button>
        </div>
    </td>
    ...
    <div className='px-5'>
      ...
      <div className='text-left py-10 space-y-3'>
        ...
        <span className='font-semibold text-black-sub'>
          There are <span className='text-tertiary-100'>{cartItems.length} </span> products in your cart
        </span>
      </div>
      <div className='flex flex-col lg:flex-row justify-between gap-5'>
        <div className='relative lg:w-[70%]  h-[300px] overflow-auto cart-scroll  shadow-md sm:rounded-lg'>
            <div className={`${!cartItems.length? 'block': 'hidden'} absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2`}>
            {CART_EMPTY}
            </div>
        ...
        <div className='flex flex-col  lg:w-[30%] border rounded-md p-5  space-y-3 shadow-md '>
          <div className='flex justify-between border rounded-md border-black-sub p-3'>
            ...
            <span className='font-bold text-lg text-tertiary-100'>₦{numberWithCommas(cartTotal)}</span>
          </div>
