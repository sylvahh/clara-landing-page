import { NO_DATA, itemsToShow, productLoading, storeNameLoading } from '../../utilities';
...
const Store = () => {
  ...
  const ProductList = !isLoading && !CproductList.length? NO_DATA : CproductList.map(({ id, img, p_name, s_new_price, s_old_price }) => (
    <Products
      key={id}
      id={id}
      image={img}
    ...
    <div className='px-5 '>
      ...
      <div className='flex flex-col p-10 sm:p-20  justify-center rounded-2xl bg-alt-tertiary  w-full h-full  space-y-5'>
        <h1 className='font-extrabold text-left  sm:text-[48px] sm:leading-[56px] text-alt-sec p-1  '>
          {!isLoading ? storeName : storeNameLoading}
        </h1>
      </div>
      ...
      <div className='mt-10 sm:px-20'>
        ...
        <div className='relative grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 my-10 '>
          {!isLoading? ProductList : loadingList}
        </div>
      </div>
    </div>
  );
