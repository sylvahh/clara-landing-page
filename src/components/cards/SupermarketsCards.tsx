...
import { arrowRight, getStoreById, imageBaseUrl } from '../../utilities';
...
const SupermarketsCards = (props: Props) => {
  ...
  const handleMarket = (storeId: string, storeLocation: string, storeName: string) => {
    getMarketId(storeId,storeLocation).then(() => Navigate(storeName))
  ...
  return (
    <div  className='flex flex-col justify-center items-center rounded-[14px] py-5 space-y-3  bg-cards shadow-sm shadow-black hover:shadow-black hover:shadow-md hover:-translate-y-1 transition-all'>
      ...
      <button
      onClick={()=> handleMarket(id, city, name)}
